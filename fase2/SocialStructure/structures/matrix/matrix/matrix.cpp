...
void Matrix::insert(string row, string col, int value)
{
    // añadir row y col si no existen, siempre se insertan col en ambas listas y row en ambas listas
    ListNode *row1Node = rowHeader->insert(row);
    ListNode *col1Node = colHeader->insert(col);
    ListNode *row2Node = rowHeader->insert(col);
    ListNode *col2Node = colHeader->insert(row);

    // crear nodo de matriz
    // se crean dos nodos de matriz por cada relacion de amistad
    MatrixNode *newMatrixNode1 = new MatrixNode(row, col, value);
    MatrixNode *newMatrixNode2 = new MatrixNode(col, row, value);

    // mover punteros de izquierda a derecha
    move_lr_pointers(row1Node, newMatrixNode1);
    move_lr_pointers(row2Node, newMatrixNode2);

    // mover punteros de arriba a abajo
    move_ub_pointers(col1Node, newMatrixNode1);
    move_ub_pointers(col2Node, newMatrixNode2);
}
...
string Matrix::get_content()
{
    ...
    linkNodes << "  \"MTX\" -> \"r" << row_node->name << "\";\n";

    while (row_node)
    {
        // create row node
        createRowNodes << "  \"r" << row_node->name << "\" [label = \"r" << row_node->name
                       << "\"  style = filled, fillcolor = bisque1, group = 0 ];\n";

        // align row node
        alignRows << "  { rank = same; \"r" << row_node->name << "\";";

        // link next row node
        if (row_node->next)
        {
            linkNodes << "  \"r" << row_node->name << "\" -> \"r" << row_node->next->name << "\";\n";
        }

        // link row node to first matrix node
        current_mtx_node = row_node->access;
        address = get_address_memory(current_mtx_node);
        linkNodes << "  \"r" << row_node->name << "\" -> \"" << address << "\";\n";
...
void Matrix::align_col_nodes(stringstream &createNodes, stringstream &align, stringstream &linkNodes)
{
    ...
    {
        // create column node
        createNodes << "  \"c" << col_hdr_node->name << "\" [label = \"c" << col_hdr_node->name
                    << "\"  style = filled, fillcolor = bisque1, group = " << col_hdr_node->name << " ];\n";

        // align column node
        align << "\"c" << col_hdr_node->name << "\";";

        // link next and prev nodes
        if (col_hdr_node->next)
        {
            linkNodes << "  \"c" << col_hdr_node->name << "\" -> \"c" << col_hdr_node->next->name << "\";\n";
        }
